...
describe("text-formatter", () => {
	describe("convertTag", () => {
		...
		it("should not convert tags with different prefix", () => {
			...
			expect(convertTag(text, "#work/")).toBe(
				"Task with #project/alpha tag",
			);
		});
	...
	describe("removeInternalLinks", () => {
		...
		it("should preserve external links", () => {
			...
			expect(removeInternalLinks(text)).toBe(
				"See [website](https://example.com) for details",
			);
		});
		...
		it("should handle multiple internal links", () => {
			const text =
				"See [note1](note1.md) and [[note2]] and [[path|alias]]";
			expect(removeInternalLinks(text)).toBe(
				"See note1 and note2 and alias",
			);
		});
	...
	describe("formatGitHubUrl", () => {
		...
		it("should format GitHub PR URL", () => {
			...
			expect(formatGitHubUrl(text)).toBe(
				"Merged [repo#456](https://github.com/owner/repo/pulls/456)",
			);
		});
		...
		it("should handle GitHub URLs with query parameters", () => {
			const text =
				"https://github.com/owner/repo/issues/123?tab=comments";
			expect(formatGitHubUrl(text)).toBe(
				"[repo#123](https://github.com/owner/repo/issues/123?tab=comments)",
			);
		});

		it("should not format URLs already in Markdown links", () => {
			const text =
				"[あのissue](https://github.com/handlename/obsidian-plugin-task-reporter/issues/1)";
			expect(formatGitHubUrl(text)).toBe(
				"[あのissue](https://github.com/handlename/obsidian-plugin-task-reporter/issues/1)",
			);
		});

		it("should not format PR URLs already in Markdown links", () => {
			const text =
				"[このPR](https://github.com/handlename/obsidian-plugin-task-reporter/pulls/2)";
			expect(formatGitHubUrl(text)).toBe(
				"[このPR](https://github.com/handlename/obsidian-plugin-task-reporter/pulls/2)",
			);
		});

		it("should format bare URLs but preserve Markdown-linked URLs", () => {
			const text =
				"Bare URL https://github.com/owner/repo/issues/1 and [linked URL](https://github.com/owner/repo/issues/2)";
			expect(formatGitHubUrl(text)).toBe(
				"Bare URL [repo#1](https://github.com/owner/repo/issues/1) and [linked URL](https://github.com/owner/repo/issues/2)",
			);
		});
	});
	...
	describe("applyStrikethrough", () => {
		...
		it("should handle text with existing formatting", () => {
			expect(applyStrikethrough("*bold* and _italic_")).toBe(
				"~*bold* and _italic_~",
			);
		});
	});
