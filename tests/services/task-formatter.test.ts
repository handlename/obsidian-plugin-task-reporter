import { formatTaskContent, formatTasks } from '../../src/services/task-formatter';
...
describe('task-formatter', () => {
	...
	describe('formatTaskContent', () => {
		...
		it('should format GitHub URLs', () => {
			...
			const result = formatTaskContent(content, defaultSettings);
			expect(result).toBe('Fixed [repo#123](https://github.com/owner/repo/issues/123)');
		});

		it('should apply all transformations in order', () => {
			const content = '#work/dev Fixed [[bug]] https://github.com/owner/repo/issues/123';
			const result = formatTaskContent(content, defaultSettings);
			expect(result).toBe('*dev* Fixed bug [repo#123](https://github.com/owner/repo/issues/123)');
		});
	...
	describe('formatTasks', () => {
		...
		it('should format multiple tasks', () => {
			const tasks: Task[] = [createTask('Task 1 #work/dev'), createTask('Task 2 #work/review')];
			const result = formatTasks(tasks, defaultSettings);
			expect(result).toBe('- Task 1 *dev*\n- Task 2 *review*');
		});
		...
		it('should handle tasks with all formatting features', () => {
			const tasks: Task[] = [
				createTask('Fixed #work/dev [[bug]] https://github.com/owner/repo/issues/123', 0, 'x'),
		...
		it('should handle tasks without tags or special formatting', () => {
			const tasks: Task[] = [createTask('Simple task 1'), createTask('Simple task 2')];
			const result = formatTasks(tasks, defaultSettings);
